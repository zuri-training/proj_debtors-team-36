...
const submit = () => {
    ...
    if (code.length < 6) {      // checks if user input is up to 6 digits
        ...
        if (code === "123456") {        // "123456" is to be changed to the OTP generated by the server. This is then cross-referenced with the user input
            location.href = 'verification-sucess.html'      // redirect to success page if the otp is correct
        }
        else {
            location.href = 'verification-fail.html'      // redirect to fail page if the otp is incorrect
        }
